...
def run_dataset(dataset_name,encoder,seed,results_suffix):
    outdir = os.path.join('results_cv', dataset_name)
    os.system('mkdir results_cv/'+dataset_name+'/npz/')
    print(dataset_name+results_suffix)
    file_prefix='results_emb_'+encoder+'_n_seed'+str(seed)+'_cv'
    data=np.load(os.path.join(outdir,'npz/'+file_prefix+str(4)+'.npz'),allow_pickle=True)
    Y=data['Y']
    # Y_pred=np.inf*np.zeros(data['size'])
    # Y_true=np.inf*np.zeros(data['size'])

    for cv_id in [4]:
        data = np.load(os.path.join(outdir,'npz/'+file_prefix+str(cv_id)+'.npz'), allow_pickle=True)
        # Y_pred[data['test_index']]=data['y_pred']
        # Y_true[data['test_index']]=data['y_true']
        Y_pred=data['y_pred']
        Y_true=data['y_true']
        # os.system('mv '+'results_cv/'+dataset_name+'/'+file_prefix+str(cv_id)+'.npz '
        #           +'results_cv/'+dataset_name+'/npz/'+file_prefix+str(cv_id)+'.npz')
    # assert np.max(np.abs(Y-Y_true))==0.0
    # print(Y_pred)
    # if not os.path.exists(outdir):
    #     os.mkdir(outdir)
    outpath = os.path.join(outdir, f'results{results_suffix}_cv5.csv')
...
def run_dataset_NMR(dataset_name,encoder,seed,results_suffix):
    outdir = os.path.join('results_cv', dataset_name)
    print(dataset_name+results_suffix)
    file_prefix='results_emb_'+encoder+'_n_seed'+str(seed)+'_strc'
...
for dataset in [
    ...
    for seed in range(10):
        print(seed)
        run_dataset(dataset, 'vae+PST+esm1b',seed,'_emb')
...
for dataset in [
    ...
    for seed in range(10):
        run_dataset(dataset, 'vae+PST+esm1b', seed, '_all_emb')
...
for dataset in [
    ...
    for seed in range(10):
        run_dataset_NMR(dataset, 'vae+PST+esm1b',seed,'_emb')
