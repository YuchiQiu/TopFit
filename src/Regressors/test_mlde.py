...
from mlde_utilis import mse,spearman
...
def process_results(all_test_loss, model_names, params,n_to_average):
    """
    ...
    Parameters
    ----------
    all_test_loss: 1d numpy array of float
    ...
    return top_models_name,top_models_params
...
X1=np.loadtxt('testing_data/elbo.npy').reshape(-1,1)
X2=np.load('testing_data/PST.npy')
Y=np.load('testing_data/Y.npy')
idx=np.arange(0,240)
X1_train=X1[idx,:]
X2_train=X2[idx,:]
Y_train=Y[idx]
idx=np.arange(755,950)
X1_test=X1[idx,:]
X2_test=X2[idx,:]
Y_test=Y[idx]
sampled_x=(X2_train,X1_train)
x_to_predict=(X2_test,X1_test)
# sampled_x=X2_train
# x_to_predict=X2_test
...
mlde_para=pd.read_csv('testing_data/TestMldeParams.csv')
limited_df = mlde_para.loc[mlde_para.Include, :]
...
if hyperopt:
    opt_trial_info,opt_training_loss,opt_testing_loss,hyperopt_params \
        = run_hyperopt_mlde(mod_hyperopts,sampled_x,Y_train,
...
# print(model_names)
# print('training loss in cv for each model')
# print(training_loss)
# print('testing loss in cv for each model')
# print(testing_loss)
...
top_models_name,top_models_params = process_results(testing_loss, model_names,best_params, n_to_average)
print(hyperopt_params)
# top_models_name = np.array([f"{model.major_model};{model.specific_model}"
#                         for model in default_mods])
# top_models_params=default_params
mlde_preds=ensemble_predictions(sampled_x,Y_train,x_to_predict,top_models_name,top_models_params,n_to_average)
