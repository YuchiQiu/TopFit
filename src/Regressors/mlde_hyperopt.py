...
from .search_spaces import SEARCH_SPACES, SPACE_BY_MODEL, CATEGORICAL_PARAMS, INTEGER_PARAMS,RENAME_VAR
...
def run_hyperopt(major_model, specific_model, training_params, 
    ...
    """
    # Get the shape of x
    x_shape = merge_tuple(sampled_x).shape
    ...
    space_var_names=[space_var if space_var not in RENAME_VAR else RENAME_VAR.get(space_var) for space_var in space_var_names]
    ...
    #                                         train_test_inds = train_test_inds)
    training_loss, testing_loss = best_model.train_cv(sampled_x, sampled_y,train_test_inds)
